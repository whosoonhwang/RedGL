...
(function () {
    ...
    var tBufferType;
    var tBuffer;
    RedBufferInfo = function (redGL, bufferType, key, pointer, dataList, pointSize, pointNum, arrayType, normalize, stride, offset, drawMode) {
        ...
        tGL.bufferData(bufferType, dataList, drawMode ? drawMode : tGL.STATIC_DRAW);
        // 정보생성
        this['shaderPointerKey'] = pointer // 쉐이더 포인터 네임
        if (arrayType) this['arrayType'] = arrayType // 타입드어레이 타입
        else throw '버퍼데이터의 형식을 지정해주세요' //TODO: 존재하지않을경우 자동 처리도 고려해보자...
        if (pointSize) this['pointSize'] = pointSize // 포인트 구성수
        else throw 'pointSize를 입력하세요'
        this['pointNum'] = pointNum ? pointNum : dataList.length.pointSize // 포인트수 
